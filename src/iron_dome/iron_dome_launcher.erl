...
%% Starts a launcher; used by sector_supervisor.
start_link(Options) -> gen_server:start_link(?MODULE, Options, []).

%% Starts an engagement; used by iron_dome_computer.
...
%% Creates the launcher state and restores its missile counter.
...
%% Handles save requests and fire requests.
...
%% Ignores messages this launcher does not use.
handle_cast(_Message, State) -> {noreply, State}.

%% Ignores process messages this launcher does not use.
handle_info(_Message, State) -> {noreply, State}.

%% Prepares one shared engagement and starts all interceptors.
launch_interceptors(TargetId, Position, Time, #{x := X} = State) ->
    StartPosition = {X, 0.0},
    %% Every interceptor uses the same meeting point and time.
    Engagement = #{
        target_id => TargetId,
...
launch_interceptors(Index, Count, Engagement, Started,
    ...
    %% Keep the data needed to move or restore the interceptor.
    Options = Engagement#{
        id => InterceptorId,
    ...
    case sector_controller:start_child(SectorController, InterceptorId, iron_dome_missile,
        ...
        {ok, InterceptorPid} ->
            %% Register this interceptor before starting the next one.
